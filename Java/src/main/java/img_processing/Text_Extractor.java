...
public class Text_Extractor {
	...
	public static String get_Text(String input, int modo) throws IOException {
		...
		for (int i = 0; i < listaImagenes.size(); i++) {
			...
			output_file = input_file.substring(0, input_file.lastIndexOf(MenuPrincipal.getSeparador()) + 1) + "output"

					+ input_file.substring(input_file.lastIndexOf(MenuPrincipal.getSeparador()),
							input_file.lastIndexOf("."));

			String commands = "tesseract " + input_file + " " + output_file;

			Metodos.crearFichero(output_file, "", false);
